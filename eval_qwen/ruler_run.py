...
"""
RULER (128K) 跑分（Tinker + Qwen）+ 时间 / 日志 / 花费统计

Follow GAM 流程，使用 Tinker API + Qwen 模型。适配 RULER 数据集（如 RULER-131072 即 128K 上下文）。
流程与 eval/ruler_run_bedrock 一致，仅将 ClaudeGenerator 换为 TinkerGenerator。

运行示例（在项目根目录）：
  单文件: python -m eval_qwen.ruler_run --data data/ruler/cwe.jsonl --outdir results_qwen/ruler
  全部数据: python -m eval_qwen.ruler_run --data data/ruler --outdir results_qwen/ruler
  （传目录时会自动遍历 data/ruler 下所有 .jsonl，不含 qa_1/qa_2）
"""
...
def get_ruler_metric_category(dataset_name: str) -> str:
    """数据集对应 RULER 三类指标之一：Retri, MT, AGG（不含 QA）。"""
...
def compute_ruler_four_metrics(
    ...
    """
    按 RULER 三类指标聚合：Retri. Acc., MT Acc., AGG. Acc.（不含 QA）
    dataset_stats: dataset_name -> {total, correct, accuracy, ...}
    返回: {"retri_acc", "mt_acc", "agg_acc"}，某类无数据则为 None。
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RULER 128K (Tinker+Qwen): time, log, cost；输出 batch_results + batch_statistics + experiment_log.jsonl"
    )
    parser.add_argument("--data", type=str, required=True, help="RULER JSONL 文件或目录（如 data/ruler/qa_1.jsonl 或 data/ruler/）")
    parser.add_argument("--outdir", type=str, default="./results_qwen/ruler", help="输出目录（下按数据集名建子目录）")
    ...
    parser.add_argument(
        ...
        default="all",
        help="只跑指定指标的数据集：retri=Retri. Acc., mt=MT Acc., agg=AGG. Acc., all=全部",
    )
    ...
    for jsonl_file in jsonl_files:
        ...
        if not indices:
            continue
        print(f"\nRULER (128K) Qwen data: {jsonl_file}, dataset={dataset_name}, samples {indices[0]}..{indices[-1]} (total {len(indices)})")
        results = []
    ...
    print("\n" + "=" * 60)
    print("RULER (128K) Qwen — 时间 / Token / 花费")
    print("=" * 60)
    ...
    print(f"\n总体汇总（含三类指标）已保存: {overall_summary_file}")

    log_experiment_run(
        outdir=args.outdir,
        bench="RULER_128K-Qwen",
