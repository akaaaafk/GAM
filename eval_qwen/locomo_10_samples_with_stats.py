...
"""
LoCoMo 10 样本评估（Tinker + Qwen）+ 统计增强版

Backbone: Tinker API; Model: Qwen/Qwen3-30B-A3B-Instruct-2507.
输出与 eval/locomo_10_samples_with_stats 一致：time，log，f1，bleu，花的钱（cost）。

# 运行示例（在 general-agentic-memory-claude 项目根下）
python -m eval_qwen.locomo_10_samples_with_stats --data data/locomo/locomo10.json --outdir ./results_qwen/locomo
"""
...
def process_sample_qwen(
    ...
    retriever: str = "both",
    save_research_trace: bool = True,
) -> List[Dict[str, Any]]:
    """使用 Tinker + Qwen 处理单个样本；记录各阶段耗时与 usage。与 eval/locomo_test 流程一致，可选保存 research_trace。"""
    ...
    memory_generator = TinkerGenerator(memory_cfg)
    if usage_containers:
    ...
    research_generator = TinkerGenerator(research_cfg)
    working_cfg = {
        "api_key": api_key,
        "model": model,
        "max_tokens": 256,
    }
    working_generator = TinkerGenerator(working_cfg)
    if usage_containers:
    ...
    for i, qi in enumerate(qas, 1):
        ...
        try:
            ...
            if save_research_trace:
                ...
                qa_item["research_trace_file"] = trace_file
                qa_item["iterations"] = len(result.raw_memory.get("iterations", []))
                all_research_traces.append({"question_index": i, "question": q, "category": cat, "research_trace": research_trace})
            qa_results.append(qa_item)
        except Exception as e:
            qa_results.append({"question": q, "gold_answer": gold, "category": cat, "error": str(e)})

    if save_research_trace and all_research_traces:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="LoCoMo 评估（Tinker + Qwen）+ Context Peak / 计时 / Cost&Token per Sample"
    )
    parser.add_argument("--data", type=str, default="/path/to/locomo/dataset.json", help="LoCoMo 数据路径")
    parser.add_argument("--outdir", type=str, default="./results_qwen/locomo", help="输出目录")
    parser.add_argument("--start-idx", type=int, default=None, help="开始样本索引（与 eval/locomo_test 一致）")
    parser.add_argument("--end-idx", type=int, default=None, help="结束样本索引（不包含）；与 --max-samples 二选一")
    parser.add_argument("--max-samples", type=int, default=10, help="处理样本数；-1 表示全部；未指定 --start-idx/--end-idx 时生效")
    parser.add_argument("--max-questions", type=int, default=None, help="每样本最多回答问题数")
    parser.add_argument("--no-research-trace", action="store_true", help="不保存 research_trace_q*.json 与 all_research_traces.json")
    parser.add_argument("--api-key", type=str, default=TINKER_API_KEY, help="Tinker API key（或 set TINKER_API_KEY）")
    parser.add_argument("--model", type=str, default=QWEN_MODEL, help="模型名，如 Qwen/Qwen3-30B-A3B-Instruct-2507")
    parser.add_argument(
        "--retriever",
        type=str,
    ...
    parser.add_argument("--price-input", type=float, default=0.0, help="输入单价（美元/百万 token），Qwen 按实际填写")
    parser.add_argument("--price-output", type=float, default=0.0, help="输出单价（美元/百万 token），Qwen 按实际填写")
    ...
    if args.start_idx is not None and args.end_idx is not None:
        start_idx, end_idx = args.start_idx, min(args.end_idx, len(samples))
        if start_idx < 0 or start_idx >= len(samples) or start_idx >= end_idx:
            print("错误: --start-idx / --end-idx 无效")
            return
        sample_slice = list(range(start_idx, end_idx))
        samples = [samples[i] for i in sample_slice]
        n_samples = len(samples)
        start_idx_out, end_idx_out = start_idx, end_idx - 1
    else:
        n_samples = len(samples) if args.max_samples < 0 else min(args.max_samples, len(samples))
        samples = samples[:n_samples]
        sample_slice = list(range(n_samples))
        start_idx_out, end_idx_out = 0, n_samples - 1
    print(f"数据: {args.data}")
    print(f"样本数: {n_samples}")
    print(f"输出目录: {args.outdir}")
    print(f"Backbone: Tinker | Model: {args.model}")
    print("=" * 60)
    ...
    all_results = []
    for ii, sample in enumerate(tqdm(samples, desc="样本")):
        actual_idx = sample_slice[ii] if sample_slice is not None else ii
        all_results.extend(
            process_sample_qwen(
                sample,
                actual_idx,
                ...
                save_research_trace=not args.no_research_trace,
            )
        )
    ...
    start_idx, end_idx = start_idx_out, end_idx_out
    statistics = {
        ...
        "running_time_sec": round(experiment_total_sec, 4),
