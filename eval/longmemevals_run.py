...
"""
GAM 在 LongMemEvals 上跑分（AWS Bedrock Inference / Claude）

LongMemEvals: 基于 LongMemEval 的评测流程
...
4. 输出 jsonl：每行 {"question_id": str, "hypothesis": str}，供 LongMemEvals 评测
...
def sessions_to_memory_messages(haystack_sessions: List[List[Dict[str, Any]]]) -> List[str]:
    """
    将 LongMemEvals 的 haystack_sessions 转为 GAM MemoryAgent 可 memorize 的文本列表。
...
def process_one_instance(
    ...
    """
    对单个 LongMemEvals instance 跑 GAM（Bedrock Claude）：建记忆 -> Research -> 生成 hypothesis。
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GAM on LongMemEvals (AWS Bedrock Claude): build memory from haystack_sessions, research question, output hypothesis jsonl."
    )
    parser.add_argument("--data", type=str, required=True, help="LongMemEvals JSON（如 data/longmemevals_oracle.json）")
    parser.add_argument("--out", type=str, default="./results/longmemevals", help="输出目录")
    ...
    data = load_longmemevals(args.data)
    ...
    print(f"LongMemEvals data: {args.data}, instances {args.start_idx}..{end-1} (total {len(indices)})")
    print(f"Output: {hyp_path}, retriever={args.retriever}")

    usage_containers = {"memory": [], "research": [], "working": []}

    with open(hyp_path, "w", encoding="utf-8") as f:
        for i in tqdm(indices, desc="GAM LongMemEvals (Bedrock)"):
            instance = data[i]
    ...
    print("\n" + "=" * 60)
    print("LongMemEvals — Token / Context Window Peak")
    print("=" * 60)
    ...
    print("To evaluate:")
    print(f"  python -m eval.longmemevals_eval_bedrock --hypothesis {os.path.abspath(hyp_path)} --oracle <path/to/longmemevals_oracle.json>")
